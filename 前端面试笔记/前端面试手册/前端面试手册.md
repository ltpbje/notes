...
假设你有一个特性分支 `feature` 和一个主分支 `master`：
...
- **使用 `merge`**：
  ...
  这会在 `master` 分支上创建一个新的合并提交，包含 `feature` 分支的所有变更。
...
- **使用 `rebase`**：
  ...
  这会将 `feature` 分支上的提交重新应用到 `master` 分支的最新状态上，生成新的提交历史。

### 总结

- 使用 `merge` 时，你会得到一个包含所有分支历史的完整历史记录，适用于合并公共分支。
- 使用 `rebase` 时，你会得到一个干净、线性的历史记录，适用于在本地开发时整理特性分支的历史。

选择使用 `rebase` 还是 `merge` 取决于你的具体需求和团队的协作方式。
...
**数据变化了, 视图自动更新** => ViewModel 底层会做好监听Object.defineProperty（vue2）Proxy(vue3)，当数据变化时，View 层会自动更新
...
一旦最高价变化了, 然后通知给其它竞价者(watcher观察者 - 订阅者, 订阅了价格的变化)，这就是**一个观察者模式** 
...
2. params传参, 不会显示在地址栏中, 刷新会丢失
   ...
   ​      (获  取参数的逻辑, 优先从$route中拿, 如果拿不到(说明刷新了), 从本地取即可)
...
3. 预渲染 prerender-spa-plugin (插件)
   ...
   预渲染是一个非常好的方式, 预渲染会在构建时, 简单的针对特定路由, 生成静态 HTM L 文件 (打包时可以帮你解析静态化)
